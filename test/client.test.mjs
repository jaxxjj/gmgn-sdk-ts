...
async function fixture(t, reply, config = {}) {
  ...
  const server = createServer(async (req, res) => {
    ...
    const call = { method: req.method, url: new URL(req.url, "http://localhost"),
...
test("library import has no CLI, network or credential-loading side effects", () => {
  ...
  `;
  const p = spawnSync(process.execPath, ["--input-type=module", "-e", script, "--", "--unexpected-cli-flag"],
...
test("smart-money and token pool routing", async t => {
  ...
  assert.deepEqual(calls.map(c => c.url.pathname), ["/v1/user/smartmoney", "/v1/token/pool_info"]);
...
test("array query parameters remain repeated keys; cursor is not interpreted", async t => {
  ...
  const result = await client.getWalletActivity("sol", "wallet", {
    type: ["buy", "sell"], cursor: "prior/+==", limit: 20,
  });
...
test("read-only POST payloads preserve provider field names", async t => {
  ...
  assert.deepEqual(JSON.parse(calls[0].body), { chain: "sol", period: "7d", wallet_addresses: ["a", "b"] });
  assert.deepEqual(JSON.parse(calls[1].body), { chain: "bsc", groups: [{ signal_type: [12], mc_min: 10 }] });
});
...
test("business errors and malformed JSON are rejected", async t => {
  ...
  await assert.rejects(client.getKol("sol"), /BAD_QUERY/);
  const invalid = await fixture(t, () => ({ raw: "not JSON" }));
  await assert.rejects(invalid.client.getKol("sol"), /non-JSON/);
});

test("401 is not retried", async t => {
  const { client, calls } = await fixture(t, () => ({ status: 401, body: { code: 401, message: "unauthorized" } }));
  await assert.rejects(client.getKol("sol"), /401/);
  assert.equal(calls.length, 1);
});

test("upstream limitation: async response rejection currently bypasses 429 retry", async t => {
  const { client, calls } = await fixture(t, (_, attempt) => attempt === 1 ? {
    status: 429,
    headers: { "x-ratelimit-reset": String(Math.floor(Date.now() / 1000)) },
    body: { code: 429, error: "RATE_LIMIT_EXCEEDED" },
  } : ok({ list: [] }));
  // Characterization, not endorsement: executePreparedRequest returns the
  // async parseResponse promise without await, so its catch does not handle it.
  // Preserve this test and invert its expectations only with an explicit vendor patch.
...
test("separate calls receive distinct authentication request IDs", async t => {
  ...
  await client.getKol("sol");
  assert.notEqual(calls[0].url.searchParams.get("client_id"), calls[1].url.searchParams.get("client_id"));
});

test("long cooldown does not block for minutes or retry", async t => {
  const { client, calls } = await fixture(t, () => ({
    status: 429, headers: { "x-ratelimit-reset": String(Math.floor(Date.now() / 1000) + 600) },
...
test("signed endpoints reject missing key without sending a request", async t => {
  const { client, calls } = await fixture(t, () => ok({}));
  await assert.rejects(client.getFollowWallet("sol"), /PRIVATE_KEY/);
  await assert.rejects(client.getWalletHoldings("sol", "wallet"), /PRIVATE_KEY/);
...
test("Ed25519 signed read verifies independently with node crypto", async t => {
  ...
  assert.ok(verify(null, Buffer.from(signatureMessage(calls[0])), publicKey,
...
test("RSA-PSS signed read verifies independently", async t => {
  ...
  assert.ok(verify("sha256", Buffer.from(signatureMessage(calls[0])), {
...
test("signed writes are not retried; test never leaves localhost", async t => {
  ...
  await assert.rejects(client.cancelStrategyOrder({
    chain: "sol", from_address: "mock-wallet", order_id: "mock-order",
  }), /429/);
  assert.equal(calls.length, 1);
